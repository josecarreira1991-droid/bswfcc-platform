    ...
    <div className="bg-white rounded-lg p-6 border border-accent/15 hover:border-accent/30 transition-all group">
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-lg font-semibold text-corp-text group-hover:text-accent transition-colors">{name}</h3>
          <p className="text-accent text-sm font-medium">{role}</p>
          {company && <p className="text-corp-muted text-sm mt-1">{company}</p>}
          <p className="text-corp-muted text-sm mt-2 line-clamp-3">{profile}</p>
          {linkedin && (
            <a
